// <copyright file="QuickSortAsync.cs" company="Andrey Pudov">
...
namespace Fundamentals.Sorting
{
    ...
    /// </summary>
    public class QuickSortAsync : ISort
    {
        /// <inheritdoc />
            ...
            where T : IComparable<T>
        {
            ...
            SortAsync(array, 0, array.Length - 1, (int)Math.Log(Environment.ProcessorCount, 2.0) + 4)
                .ConfigureAwait(false);
        }

        private static async Task SortAsync<T>(T[] array, int lo, int hi, int depth)
            where T : IComparable<T>
        {
            if (lo < hi)
            {
                ...
                {
                    await SortAsync(array, lo, p - 1, depth - 1);
                    await SortAsync(array, p + 1, hi, depth - 1);
                }
            }
        }
