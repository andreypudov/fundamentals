...
type ConditionalOperator<'T when 'T :> IConditionalOperator and 'T : (new : unit -> 'T)>() =
    member this.operator = new 'T()

    [<Benchmark>]
    [<Arguments(Color.White)>]
    member this.First color = color |> this.operator.GetName

    [<Benchmark>]
    [<Arguments(Color.Purple)>]
    member this.Last color = color |> this.operator.GetName

    [<Benchmark>]
    [<Arguments(Color.Black)>]
    member this.MinValue color = color |> this.operator.GetName